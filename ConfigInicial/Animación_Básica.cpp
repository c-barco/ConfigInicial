...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(0.0f, movBall, 0.09f));
...
void Animation() {
	if (AnimBall)
	{
		float limiteInferior = -0.1f;
		float limiteSuperior = pointLightPositions[0].y;
		float velocidad = 1.5f;

		if (up)
		{
			movBall += velocidad * deltaTime;
			if (movBall >= limiteSuperior) up = false;
		}
		else
		{
			movBall -= velocidad * deltaTime;
			if (movBall <= limiteInferior) up = true;
		}
	}
}
